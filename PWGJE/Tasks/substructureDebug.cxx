...
struct SubstructureDebugTask {
  ...
  void processDataD0(aod::D0CJetCOs::iterator const&, soa::Join<aod::D0CJetOs, aod::D0CJetSSOs> const& jets, CandidatesD0Data const&)
  {
    for (auto const& jet : jets) {
      ...
      auto const& candidate = jet.candidate_as<CandidatesD0Data>();
      registry.fill(HIST("h_D0injet_pt"), candidate.pt());
    }
  }
  PROCESS_SWITCH(SubstructureDebugTask, processDataD0, "jets D0 data", true);

  void processDataLc(aod::LcCJetCOs::iterator const&, soa::Join<aod::LcCJetOs, aod::LcCJetSSOs> const& jets, CandidatesLcData const&)
  {
    for (auto const& jet : jets) {
      ...
      auto const& candidate = jet.candidate_as<CandidatesLcData>();
  ...
  void processMCDD0(aod::D0CMCDJetCOs::iterator const&, soa::Join<aod::D0CMCDJetOs, aod::D0CMCDJetSSOs, aod::D0CMCDJetMOs> const& jets, aod::D0CMCPJetOs const&, CandidatesD0MCD const&)
  {
    for (auto const& jet : jets) {
      registry.fill(HIST("h_D0jet_pt"), jet.jetPt());
      registry.fill(HIST("h_D0jet_phi"), jet.jetPhi());
      ...
      registry.fill(HIST("h_D0jet_nTracks"), jet.jetNConstituents());
      registry.fill(HIST("h_D0jet_angularity"), jet.angularity());
      auto const& matchedJets = jet.matchedJetCand_as<aod::D0CMCPJetOs>();
      for (auto const& mcpjet : matchedJets) {
        registry.fill(HIST("h_D0jet_pt_matched"), mcpjet.jetPt(), jet.jetPt());
      }
      auto const& candidate = jet.candidate_as<CandidatesD0MCD>();
      registry.fill(HIST("h_D0injet_pt"), candidate.pt());
    }
  }
  PROCESS_SWITCH(SubstructureDebugTask, processMCDD0, "jets D0 mcd", false);

  void processMCDLc(aod::LcCMCDJetCOs::iterator const&, soa::Join<aod::LcCMCDJetOs, aod::LcCMCDJetSSOs, aod::LcCMCDJetMOs> const& jets, aod::LcCMCPJetOs const&, CandidatesLcMCD const&)
  {
    for (auto const& jet : jets) {
      registry.fill(HIST("h_Lcjet_pt"), jet.jetPt());
      registry.fill(HIST("h_Lcjet_phi"), jet.jetPhi());
      ...
      registry.fill(HIST("h_Lcjet_nTracks"), jet.jetNConstituents());
      registry.fill(HIST("h_Lcjet_angularity"), jet.angularity());
      auto const& matchedJets = jet.matchedJetCand_as<aod::LcCMCPJetOs>();
      for (auto const& mcpjet : matchedJets) {
        registry.fill(HIST("h_Lcjet_pt_matched"), mcpjet.jetPt(), jet.jetPt());
      }
      auto const& candidate = jet.candidate_as<CandidatesLcMCD>();
      registry.fill(HIST("h_Lcinjet_pt"), candidate.pt());
    }
  }
  PROCESS_SWITCH(SubstructureDebugTask, processMCDLc, "jets Lc mcd", false);

  void processMCPCharged(soa::Join<aod::CMCPJetOs, aod::CMCPJetSSOs, aod::CMCPJetMOs>::iterator const& jet)
  {
  ...
  void processMCPD0(soa::Join<aod::D0CMCPJetOs, aod::D0CMCPJetSSOs, aod::D0CMCPJetMOs>::iterator const& jet, CandidatesD0MCP const&)
  {
    ...
    auto const& candidate = jet.candidate_as<CandidatesD0MCP>();
    registry.fill(HIST("h_mcpD0injet_pt"), candidate.pt());
  }
  PROCESS_SWITCH(SubstructureDebugTask, processMCPD0, "jets D0 mcp", false);

  void processMCPLc(soa::Join<aod::LcCMCPJetOs, aod::LcCMCPJetSSOs, aod::LcCMCPJetMOs>::iterator const& jet, CandidatesLcMCP const&)
  {
    ...
    auto const& candidate = jet.candidate_as<CandidatesLcMCP>();
