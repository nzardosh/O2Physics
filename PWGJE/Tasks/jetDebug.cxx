...
struct JetDebugTask {
  ...
  void processCollisionSelection(JetCollisions const& collisions, soa::Join<aod::Collisions, aod::EvSels> const& originalCollisions)
  {
    for (auto const& collision : collisions) {
      if (jetderiveddatautilities::selectCollision(collision, jetderiveddatautilities::initialiseEventSelection(static_cast<std::string>("sel8")))) {
  ...
  void processTrackSelection(JetTracks const& tracks, soa::Join<aod::Tracks, aod::TrackSelection> const& originalTracks)
  {
  ...
  void processDataCharged(JetCollision const& collision, soa::Join<aod::ChargedJets, aod::ChargedJetConstituents> const& jets, aod::JTracks const& tracks)
  {
    ...
    for (auto const& jet : jets) {
      ...
      for (auto const& jetConstituent : jet.tracks_as<aod::JTracks>()) {
  ...
  void processDataD0(JetCollision const&, soa::Join<aod::D0ChargedJets, aod::D0ChargedJetConstituents> const& jets, aod::JTracks const&, CandidatesD0Data const& candidates)
  {
    ...
    for (auto const& jet : jets) {
      ...
      for (auto const& jetCandidate : jet.candidates_as<CandidatesD0Data>()) {
  ...
  void processDataLc(JetCollision const&, soa::Join<aod::LcChargedJets, aod::LcChargedJetConstituents> const& jets, aod::JTracks const&, CandidatesLcData const& candidates)
  {
    for (auto const& candidate : candidates) {
    ...
    for (auto const& jet : jets) {
      ...
      for (auto const& jetCandidate : jet.candidates_as<CandidatesLcData>()) {
  ...
  void processMCDCharged(JetCollision const& collision, soa::Join<aod::ChargedMCDetectorLevelJets, aod::ChargedMCDetectorLevelJetConstituents, aod::ChargedMCDetectorLevelJetsMatchedToChargedMCParticleLevelJets> const& jets, aod::JTracks const& tracks, aod::ChargedMCParticleLevelJets const&)
  {
    registry.fill(HIST("h_collisions"), 1.0);
    if (jetderiveddatautilities::selectCollision(collision, eventSelection)) {
      registry.fill(HIST("h_collisions"), 2.0);
  ...
  void processMCDD0(JetCollision const&, soa::Join<aod::D0ChargedMCDetectorLevelJets, aod::D0ChargedMCDetectorLevelJetConstituents, aod::D0ChargedMCDetectorLevelJetsMatchedToD0ChargedMCParticleLevelJets> const& jets, aod::JTracks const&, CandidatesD0MCD const& candidates, aod::D0ChargedMCParticleLevelJets const&)
  {
    for (auto const& candidate : candidates) {
      registry.fill(HIST("h_D0_pt"), candidate.pt());
    }
    for (auto const& jet : jets) {
      ...
      for (auto const& jetCandidate : jet.candidates_as<CandidatesD0MCD>()) {
        registry.fill(HIST("h_D0injet_pt"), jetCandidate.pt());
        angularity += (jetCandidate.pt() * TMath::Sqrt(TMath::Power(RecoDecay::constrainAngle(jet.phi() - jetCandidate.phi(), -M_PI), 2.0) + TMath::Power(jet.eta() - jetCandidate.eta(), 2.0))) / (jet.pt() * (jet.r() / 100.f));
      }
      registry.fill(HIST("h_D0jet_angularity"), angularity);
  ...
  void processMCDLc(JetCollision const&, soa::Join<aod::LcChargedMCDetectorLevelJets, aod::LcChargedMCDetectorLevelJetConstituents, aod::LcChargedMCDetectorLevelJetsMatchedToLcChargedMCParticleLevelJets> const& jets, aod::JTracks const&, CandidatesLcMCD const& candidates, aod::LcChargedMCParticleLevelJets const&)
  {
    for (auto const& candidate : candidates) {
      registry.fill(HIST("h_Lc_pt"), candidate.pt());
    }
    for (auto const& jet : jets) {
      ...
      for (auto const& jetCandidate : jet.candidates_as<CandidatesLcMCD>()) {
        registry.fill(HIST("h_Lcinjet_pt"), jetCandidate.pt());
        angularity += (jetCandidate.pt() * TMath::Sqrt(TMath::Power(RecoDecay::constrainAngle(jet.phi() - jetCandidate.phi(), -M_PI), 2.0) + TMath::Power(jet.eta() - jetCandidate.eta(), 2.0))) / (jet.pt() * (jet.r() / 100.f));
      }
      registry.fill(HIST("h_Lcjet_angularity"), angularity);
  ...
  void processMCPD0(aod::JMcCollision const&, soa::Join<aod::D0ChargedMCParticleLevelJets, aod::D0ChargedMCParticleLevelJetConstituents> const& jets, aod::JMcParticles const&, CandidatesD0MCP const& candidates)
  {
    for (auto const& candidate : candidates) {
    ...
    for (auto const& jet : jets) {
      ...
      for (auto const& jetCandidate : jet.candidates_as<CandidatesD0MCP>()) {
  ...
  void processMCPLc(aod::JMcCollision const&, soa::Join<aod::LcChargedMCParticleLevelJets, aod::LcChargedMCParticleLevelJetConstituents> const& jets, aod::JMcParticles const&, CandidatesLcMCP const& candidates)
  {
    for (auto const& candidate : candidates) {
    ...
    for (auto const& jet : jets) {
      ...
      for (auto const& jetConstituent : jet.tracks_as<aod::JMcParticles>()) {
        angularity += (jetConstituent.pt() * TMath::Sqrt(TMath::Power(RecoDecay::constrainAngle(jet.phi() - jetConstituent.phi(), -M_PI), 2.0) + TMath::Power(jet.eta() - jetConstituent.eta(), 2.0))) / (jet.pt() * (jet.r() / 100.f));
        for (auto const& jetCandidate : jet.candidates_as<CandidatesLcMCP>()) {
