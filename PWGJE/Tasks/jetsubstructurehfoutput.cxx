    ...
    for (const auto& jet : jets) {
      ...
      for (const auto& jetRadiiValue : jetRadiiValues) {
        if (jet.r() == round(jetRadiiValue * 100.0f)) {
          ...
          int32_t candidateIndex = -1;
          if constexpr (isMCP) {
            auto hfMcCollisionIndex = candidateMcCollisionMapping.find(jetcandidateutilities::getCandidateMcCollisionId(candidate));
    ...
    if constexpr (!isMCPOnly) {
      for (const auto& collision : collisions) {
        if (collisionFlag[collision.globalIndex()]) {
          const auto hfCollisionsPerCollision = jetcandidateutilities::slicedPerCandidateCollision(hfCollisions, candidates, collision, D0CollisionsPerCollision, LcCollisionsPerCollision, D0CollisionsPerCollision, DielectronCollisionsPerCollision); // add Bplus later TODO Fix for dielectrons
          int32_t candidateCollisionIndex = -1;
