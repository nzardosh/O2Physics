...
struct JetSubstructureHFTask {
  ...
  void jetPairing(T const& jet, U const& tracks, V const& candidates)
  {
    ...
    for (typename std::vector<typename V::iterator>::size_type candidate1Index = 0; candidate1Index < candidatesVec.size() - 1; candidate1Index++) {
      for (typename std::vector<typename V::iterator>::size_type candidate2Index = candidate1Index + 1; candidate2Index < candidatesVec.size(); candidate2Index++) {
    ...
    for (typename std::vector<typename V::iterator>::size_type candidateIndex = 0; candidateIndex < candidatesVec.size(); candidateIndex++) { // could just directly get the candidate and tracks here but keeping it consistent with above
      for (typename std::vector<typename U::iterator>::size_type trackIndex = 0; trackIndex < tracksVec.size(); trackIndex++) {
