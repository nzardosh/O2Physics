...
struct JetDerivedDataProducerTask {
  ...
  { // can loop over McV0Labels tables if we want to only store matched V0Particles
    ...
    for (auto const& particle : particles) {
      if (jetv0utilities::isV0Particle(particle)) {
        ...
        jV0McsTable(jV0McCollisionsTable.lastIndex(), particle.pt(), particle.eta(), particle.phi(), particle.y(), particle.e(), pdgParticle->Mass(), particle.pdgCode(), particle.getGenStatusCode(), particle.getHepMCStatusCode(), particle.isPhysicalPrimary(), jetv0utilities::setV0ParticleDecayBit<aod::McParticles>(particle));
  ...
  void processDielectronMc(aod::McCollision const& mcCollision, aod::McParticles const& particles)
  {
    ...
    for (auto const& particle : particles) {
      if (jetdqutilities::isDielectronParticle(particle)) {
