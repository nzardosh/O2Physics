...
{
  return std::is_same_v<std::decay_t<T>, CandidatesDielectronMCP> || std::is_same_v<std::decay_t<T>, o2::soa::Filtered<CandidatesDielectronMCP>>; // note not optimal way but needed for jetfindingutilities::analyseParticles()
}

// TODO: Add selections for the matching here!
/**
...
template <typename T>
bool isDielectronParticle(T const& particle)
{
  if (TMath::Abs(particle.pdgCode()) == o2::constants::physics::Pdg::kJPsi) { // JPsi
    return true;
  }
  return false;
}
...
uint8_t setDielectronParticleDecayBit(T const& particle)
{
  uint8_t bit = 0;
  ...
  if (particle.has_daughters()) {
    ...
    if (TMath::Abs(daughter1PdgCode) == 11 && TMath::Abs(daughter2PdgCode) == 11) { // ToDo should we take care of the sign?
...
{
  DielectronMcTable(mcCollisionIndex, candidate.pt(), candidate.eta(), candidate.phi(), candidate.y(), candidate.e(), candidate.m(), candidate.pdgCode(), candidate.getGenStatusCode(), candidate.getHepMCStatusCode(), candidate.isPhysicalPrimary(), candidate.decayFlag());
