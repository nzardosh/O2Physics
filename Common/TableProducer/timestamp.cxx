  ...
  void process(aod::BC const& bc)
  {
    ...
    if (runNumber == lastRunNumber) { // The run number coincides to the last run processed
      ...
      if (isRun2MC.value == 1 || isUnanchoredRun3MC) {
        ...
        auto ctp = ccdb->getSpecific<std::vector<Long64_t>>(orbit_reset_path.value.data(), sorTimestamp);
        ...
        auto ctp = ccdb->getSpecific<std::vector<Long64_t>>(orbit_reset_path.value.data(), eorTimestamp);
        orbitResetTimestamp = (*ctp)[0];
      }
